...
export function setUpCalculator () {
  ...
  function appendNumber(number) {
    console.log('appendNumber function started ' + number)
    if (dataScreen.textContent === "0" || shouldResetScreen) {
      //console.log('screen reset when appending ' + number);
      resetScreen();
    }
    dataScreen.textContent += number;
  }
  //Clear Screen Text and then resets shouldResetScreen variable
  function resetScreen() {
    dataScreen.textContent = "";
  ...
  function appendPoint() {
    ...
    if (dataScreen.textContent === "") dataScreen.textContent = "0";
    if (dataScreen.textContent.includes(".")) {
  ...
  function deleteNumber() {
    dataScreen.textContent = dataScreen.textContent.toString().slice(0, -1);
  }
  //Set operator
  function setOperation(operator) {
    console.log(operator);
    if (currentOperation !== null) evaluate(); //if already a operator stored evaluate operation and return result
    firstOperand = dataScreen.textContent;//set first number
  ...
  function evaluate() {
    ...
    if (currentOperation === "÷" && dataScreen.textContent === "0") {//does it divide by zero check
    ...
    secondOperand = dataScreen.textContent;
    console.log('secondOperand = ' + secondOperand)
    dataScreen.textContent = roundResult(
